...
def guess(data, algebra, **kwargs):
    ...
    - ``start`` -- an integer specifying at which rank does the recurrence starts.
    ...
    if A.ngens() > 1 or R.ngens() > 1:
        ...
        result = guess(data, A.change_ring(R.ring()), **kwargs)
        ...
        result = guess(data, A0, **kwargs).change_ring(R).to_F(A)

    elif (not A.is_S() and not A.is_D() and not A.is_Q() and not A.is_C()):
        raise TypeError, "unexpected algebra: " + str(A)

    elif K.is_prime_field() and K.characteristic() > 0:
        result = _guess_via_gcrd(data, A, **kwargs)

    elif K is ZZ:
        # CRA
        result = _guess_via_hom(data, A, _word_size_primes(), lambda mod : GF(mod), **kwargs)
        ...
        result = _guess_via_hom(data, A, mod, to_hom, **kwargs)
        ...
        result = _guess_via_hom(data, A, _word_size_primes(), to_hom, **kwargs)

    elif K is QQ:
        result = guess(data, A.change_ring(ZZ[x]), **kwargs)

    elif K.is_field():
        result = guess(data, A.change_ring(K.ring()[x]), **kwargs)

    elif is_PolynomialRing(K) and K.base_ring() is QQ:
        result = guess(data, A.change_ring(ZZ[K.gens()][x]), **kwargs)

    else:
        raise TypeError, "unexpected coefficient domain: " + str(K)

    if subs_flag:
        # For some reason, subs does not seem to be working for result ore_algebra elements...
        # So I use this cheap trick to work around the problem
        parent = result.parent()
        n = result.base_ring().gen()
        return parent(result.polynomial().subs(n=n-start))
    else:
        return result
